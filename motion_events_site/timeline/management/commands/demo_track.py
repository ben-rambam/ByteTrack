...
from timeline.models import TrackableClass, Trackable, Event, Tracklet
...
def imageflow_demo(predictor, vis_folder, current_time, args, exp):
    ...
    logger.info("image width: {}".format(width))
    ...
    if args.save_events:
        ...
        trackable_class_list = TrackableClass.objects.all()
        trackable_class_dict = { item.class_id: item for item in trackable_class_list } 
        trackables = {}
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        dummy = Dummy(**options)
        exp = get_exp(dummy.exp_file, dummy.name)
